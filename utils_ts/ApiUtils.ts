...
class ApiUtils {
    ...
    async createOrder(orderPayLoad:string) {     // call getToken()(loginApi) to get token in createOrder()
        let response = {token:String,orderId:String};// before adding varibale,define dataType // just create empty responce variable,so we can store token and orderId                       
